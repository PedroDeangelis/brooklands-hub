...
/**
 * Fetches Business Central posted sales credit memo headers and queues one import job per row.
 *
 ...
 *   --full         reconciliation — every posted credit memo, checkpoint ignored
 *   --top=N        manual — at most N rows, and never touches the checkpoint
 *   --number=NO    manual — that one credit memo only, and never touches the checkpoint
 *
 * Only the header is fetched here. The lines behind
 * each memo are read by the job, on Horizon, so a slow or failing read of
 * one memo's lines never holds the scheduler or the checkpoint.
 *
 * --force additionally re-delivers every fetched credit memo, whether or not
 * anything moved. That is for putting a drifted website back in step. The
 * scheduler never passes it.
 */
#[Signature('bc:import-sales-credit-memos
    {--page-size=200 : How many credit memos to request per Business Central call}
    {--top= : Stop after this many credit memos in total (manual testing; never advances the checkpoint)}
    {--full : Re-import every posted credit memo, ignoring the incremental checkpoint}
    {--force : Re-deliver every fetched credit memo even when nothing changed (requires --full)}
    {--number= : Fetch one credit memo by its Business Central number (manual testing; never advances the checkpoint)}
    {--skip=0 : How many credit memos to skip before fetching}')]
#[Description('Fetch posted sales credit memos from Business Central and queue an import job for each one')]
class ImportBcSalesCreditMemosCommand extends Command
{
    public function handle(BusinessCentralClient $client, BusinessCentralPager $pager): int
    {
        ...
        $checkpoint = SyncCheckpoint::forEntity(SyncCheckpoint::ENTITY_SALES_CREDIT_MEMOS);

        // A full run ignores the checkpoint; so does the very first run, which
        // has none. Both therefore fetch every posted credit memo.
        ...
        try {
            $result = $pager->fetch(
                ...
                SalesCreditMemosQuery::ENTITY_SET,
                fn (int $ask, int $fetched): array => SalesCreditMemosQuery::page($ask, $skip + $fetched, $since),
                function (array $row) use (&$dispatched, $force): void {
                    ImportBcSalesCreditMemo::dispatch($row, $force);
                    $dispatched++;
                },
                $pageSize,
                $limit,
            ...
            $this->warn(sprintf(
                'Fetch failed after queueing %d credit memo(s). The checkpoint was not advanced.',
                $dispatched,
            ));

            Log::error('bc.import_sales_credit_memos.failed', [
                'queued' => $dispatched,
                'since' => $since,
                'error' => $e->getMessage(),
            ]);
        ...
        if ($result->rows === 0) {
            $this->info($since === null
                ? 'No sales credit memos returned from Business Central.'
                : 'No sales credit memos have changed since the last run.');
        ...
        $this->newLine();
        $this->info(sprintf(
            'Queued %d credit memo(s) across %d request(s).',
            $result->rows,
            $result->pages,
        ));
    ...
    /**
     * Fetch and queue a single credit memo by its Business Central number.
     *
    ...
    {
        $this->line(sprintf('Fetching the single sales credit memo %s, ignoring the checkpoint.', $number));
        ...
        try {
            $response = $client->getCustom(
                ...
                SalesCreditMemosQuery::VERSION,
                SalesCreditMemosQuery::ENTITY_SET,
                SalesCreditMemosQuery::forNumber($number),
            );
        } catch (BusinessCentralException $e) {
            $this->error($e->getMessage());
        ...
        if (! is_array($rows) || $rows === []) {
            $this->warn(sprintf('No posted Business Central sales credit memo has the number "%s".', $number));
        ...
        foreach ($rows as $row) {
            ...
            ImportBcSalesCreditMemo::dispatch($row);
            $queued++;

            $this->line(sprintf('  queued %s (%s)', $row['number'] ?? $number, $row['id'] ?? 'unknown id'));
        }

        $this->info(sprintf('Queued %d credit memo(s). The checkpoint was left unchanged.', $queued));
    ...
    {
        if (! $full) {
            $this->error('--force requires --full: a forced run re-delivers every credit memo.');
    ...
    {
        $this->warn('Forcing re-delivery of every posted sales credit memo.');
    ...
    {
        if ($full) {
            $this->line('Full reconciliation: fetching every posted sales credit memo, ignoring the checkpoint.');
        } elseif ($since === null) {
            $this->line('No checkpoint yet: fetching every posted sales credit memo.');
        } else {
            $this->line(sprintf('Incremental: fetching sales credit memos changed since %s.', $since));
        }

        $this->line(sprintf(
            'Page size %d%s.',
            $pageSize,
            $limit === null ? '' : ", stopping after {$limit} credit memo(s)",
        ));
    }
}
