...
use App\BusinessCentral\ContactsQuery;
...
/**
 * Fetches Business Central person contacts and queues one import job per row.
 *
 * Only contacts of type Person are fetched (see ContactsQuery); every other
 * rule that decides whether a contact becomes a website user is applied
 * locally, so the dashboard can explain an excluded one. The customer link is
 * not on this page: bc:import-contact-links attaches it.
 *
 ...
 *   --full         reconciliation — every contact, checkpoint ignored
 *   --top=N        manual — at most N rows, and never touches the checkpoint
 *   --number=NO     manual — that one contact only, and never touches the checkpoint
 *
 * Normalisation and persistence stay in the job, so the same path runs whether
 * a row arrives from here or from anywhere else.
 *
 * --force additionally re-delivers every fetched contact that qualifies,
 * whether or not anything moved. That is for putting a drifted website back
 * in step; it never creates a user for a contact the rules exclude. The
 * scheduler never passes it.
 */
#[Signature('bc:import-contacts
    {--page-size=200 : How many contacts to request per Business Central call}
    {--top= : Stop after this many contacts in total (manual testing; never advances the checkpoint)}
    {--full : Re-import every contact, ignoring the incremental checkpoint}
    {--force : Re-deliver every fetched contact even when nothing changed (requires --full)}
    {--number= : Fetch one contact by its Business Central number (manual testing; never advances the checkpoint)}
    {--skip=0 : How many contacts to skip before fetching}')]
#[Description('Fetch contacts from Business Central and queue an import job for each one')]
class ImportBcContactsCommand extends Command
{
    public function handle(BusinessCentralClient $client, BusinessCentralPager $pager): int
    {
        ...
        $checkpoint = SyncCheckpoint::forEntity(SyncCheckpoint::ENTITY_CONTACTS);

        // A full run ignores the checkpoint; so does the very first run, which
        // has none. Both therefore fetch every contact.
        ...
        try {
            $result = $pager->fetch(
                ContactsQuery::PUBLISHER,
                ContactsQuery::GROUP,
                ContactsQuery::VERSION,
                ContactsQuery::ENTITY_SET,
                fn (int $ask, int $fetched): array => ContactsQuery::page($ask, $skip + $fetched, $since),
                function (array $row) use (&$dispatched, $force): void {
                    ImportBcContact::dispatch($row, $force);
                    $dispatched++;
                },
                $pageSize,
                $limit,
            ...
            $this->warn(sprintf(
                'Fetch failed after queueing %d contact(s). The checkpoint was not advanced.',
                $dispatched,
            ));

            Log::error('bc.import_contacts.failed', [
                'queued' => $dispatched,
                'since' => $since,
                'error' => $e->getMessage(),
            ]);
        ...
        if ($result->rows === 0) {
            $this->info($since === null
                ? 'No contacts returned from Business Central.'
                : 'No contacts have changed since the last run.');
        ...
        $this->newLine();
        $this->info(sprintf(
            'Queued %d contact(s) across %d request(s).',
            $result->rows,
            $result->pages,
        ));
    ...
    /**
     * Fetch and queue a single contact by its Business Central number.
     *
    ...
    {
        $this->line(sprintf('Fetching the single contact %s, ignoring the checkpoint.', $number));
        ...
        try {
            $response = $client->getCustom(
                ...
                ContactsQuery::VERSION,
                ContactsQuery::ENTITY_SET,
                ContactsQuery::forNumber($number),
            );
        } catch (BusinessCentralException $e) {
            $this->error($e->getMessage());
        ...
        if (! is_array($rows) || $rows === []) {
            $this->warn(sprintf('No Business Central contact has the code "%s".', $number));
        ...
        foreach ($rows as $row) {
            ...
            ImportBcContact::dispatch($row);
            $queued++;

            $this->line(sprintf('  queued %s (%s)', $row['number'] ?? $number, $row['id'] ?? 'unknown id'));
        }

        $this->info(sprintf('Queued %d contact(s). The checkpoint was left unchanged.', $queued));
    ...
    {
        if (! $full) {
            $this->error('--force requires --full: a forced run re-delivers every contact.');
    ...
    {
        $this->warn('Forcing re-delivery of every contact.');
        $this->line('Each qualifying one will be queued for WordPress even if nothing changed.');
    ...
    {
        if ($full) {
            $this->line('Full reconciliation: fetching every contact, ignoring the checkpoint.');
        } elseif ($since === null) {
            $this->line('No checkpoint yet: fetching every contact.');
        } else {
            $this->line(sprintf('Incremental: fetching contacts changed since %s.', $since));
        }

        $this->line(sprintf(
            'Page size %d%s.',
            $pageSize,
            $limit === null ? '' : ", stopping after {$limit} contact(s)",
        ));
    }
}
