...
/**
 * Delivers one customer's current desired website state.
 *
 * The customer counterpart to DeliverProductToWebsite, and a sibling rather
 ...
 * runs, so a job queued before a change cannot deliver what the customer looked
 * like then.
 */
class DeliverCustomerToWebsite implements ShouldQueue
{
    use Queueable;
    ...
    public function __construct(
        ...
        public readonly string $channel = CustomerSyncLedger::CHANNEL_WEBSITE,
    ) {}

    /**
     * Only one delivery per customer per channel at a time.
     *
     * The lock names the entity as well as the id, so a customer and a product
    ...
    public function handle(CustomerSyncLedger $ledger, WebsiteClient $client): void
    {
        ...
        $customer = Customer::query()->where('bc_id', $this->bcId)->first();

        if ($customer === null) {
            // The ledger wants something delivered for a customer that is no
            // longer here. Nothing can be rebuilt, so this needs a person.
            $ledger->markFailed($record, 'No customer exists for this ledger row.');

            return;
        }

        // Rebuild from current state rather than trusting anything queued.
        $plan = $ledger->plan($customer, $this->channel);
        ...
        try {
            $response = $client->deliver(
                WebsiteRequest::fromPlan($plan, WebsiteRequest::ENTITY_CUSTOMER),
            );
        ...
        if ($response->isDelivered()) {
            ...
            Log::info('website.customer_delivery.succeeded', $this->context($plan->type, $response->status, [
                'recorded' => $matched,
                'stale' => ! $matched,
            ]));
        ...
        if ($response->isAccepted()) {
            ...
            Log::info('website.customer_delivery.accepted_not_applied', $this->context($plan->type, $response->status));

            return;
        }

        if ($response->needsFullSync()) {
            // The website has no customer post for this customer, so the payload we
            ...
            // is how a customer whose customer post WordPress deleted comes back.
            $ledger->forgetDelivered($record);

            Log::info('website.customer_delivery.full_sync_required', $this->context($plan->type, $response->status));
        ...
        if ($response->isConflict()) {
            ...
            Log::warning('website.customer_delivery.conflict', $this->context($plan->type, $response->status, [
                'conflicts' => array_map(
        ...
        if ($response->isTransient()) {
            ...
            Log::warning('website.customer_delivery.transient', $this->context($plan->type, $response->status));
        ...
        Log::error('website.customer_delivery.failed', $this->context($plan->type, $response->status, [
    ...
    {
        return ['website-delivery', 'entity:customer', 'bc:'.$this->bcId, 'channel:'.$this->channel];
    }

    public function failed(?Throwable $exception): void
    {
        Log::error('website.customer_delivery.exhausted', [
            'bc_id' => $this->bcId,
    ...
    {
        return "website-delivery:{$this->channel}:customer:{$this->bcId}";
    }
    ...
    private function context(DeliveryType $type, ?int $status, array $extra = []): array
    {
        return array_merge([
            'bc_id' => $this->bcId,
            'channel' => $this->channel,
            'entity' => CustomerSyncLedger::ENTITY_CUSTOMER,
            'mode' => $type->value,
            'status' => $status,
        ], $extra);
    }
