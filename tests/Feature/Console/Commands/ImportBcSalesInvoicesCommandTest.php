...
/**
 * Incremental posted invoice synchronisation.
 *
 * The behaviour that matters is when the command stops fetching and when it is
 * allowed to move the checkpoint, exactly as for orders. Only the header is
 * fetched here; the lines are the job's concern.
 */
class ImportBcSalesInvoicesCommandTest extends TestCase
{
    use LazilyRefreshDatabase;

    private const TOKEN_URL = 'https://login.microsoftonline.com/tenant-abc/oauth2/v2.0/token';

    private const INVOICES_URL = 'https://api.businesscentral.dynamics.com/v2.0/tenant-abc/Sandbox_Test'
        .'/api/brooklands/catalog/v1.0/companies(company-guid)/postedSalesInvoicesExt';
    ...
    protected function setUp(): void
    {
        ...
        Queue::fake([ImportBcSalesInvoice::class]);
    }
    ...
    private function row(int $n, string $modified = '2026-06-15T00:23:21.653Z'): array
    {
        return [
            ...
            'number' => sprintf('INV%06d', $n),
            'orderNumber' => sprintf('SO%06d', $n),
    ...
    {
        return SyncCheckpoint::forEntity(SyncCheckpoint::ENTITY_SALES_INVOICES)->refresh();
    }
     ...
     */
    private function fakeInvoices(array $rows): void
    {
        Http::fake([
            self::TOKEN_URL => Http::response(['access_token' => 'tok', 'expires_in' => 3599]),
            self::INVOICES_URL.'*' => function (Request $request) use ($rows) {
    ...
    private function sentFilters(): array
    {
        ...
        Http::assertSent(function (Request $request) use (&$filters): bool {
            if (str_starts_with($request->url(), self::INVOICES_URL)) {
    ...
    public function test_a_first_run_fetches_every_invoice(): void
    {
        $this->fakeInvoices($this->rows(3));

        $this->artisan('bc:import-sales-invoices', ['--page-size' => 200])
            ->expectsOutputToContain('No checkpoint yet')
            ->assertExitCode(0);

        Queue::assertPushed(ImportBcSalesInvoice::class, 3);
    }

    public function test_a_first_run_sends_no_since_filter(): void
    {
        $this->fakeInvoices($this->rows(2));

        $this->artisan('bc:import-sales-invoices', ['--page-size' => 200])->assertExitCode(0);
    ...
    public function test_the_command_reads_the_header_page_only(): void
    {
        ...
        Http::assertSent(function (Request $request): bool {
            if (str_starts_with($request->url(), self::INVOICES_URL)) {
                ...
                $this->assertStringContainsString('orderNumber', $request['$select']);
            }

            $this->assertStringNotContainsString('salesInvoiceLines', $request->url());
    ...
    {
        $this->fakeInvoices($this->rows(5));

        $this->artisan('bc:import-sales-invoices', ['--page-size' => 2])->assertExitCode(0);

        Queue::assertPushed(ImportBcSalesInvoice::class, 5);
    ...
    public function test_a_second_run_does_not_refetch_the_boundary_invoice(): void
    {
        $this->fakeInvoices([$this->row(1, '2026-06-15T00:23:21.653Z')]);
        $this->artisan('bc:import-sales-invoices', ['--page-size' => 200])->assertExitCode(0);

        Queue::assertPushed(ImportBcSalesInvoice::class, 1);

        $this->artisan('bc:import-sales-invoices', ['--page-size' => 200])
            ->expectsOutputToContain('No sales invoices have changed')
            ->assertExitCode(0);
    ...
    public function test_a_capped_run_never_advances_the_checkpoint(): void
    {
        ...
        $this->artisan('bc:import-sales-invoices', ['--page-size' => 200, '--top' => 2])
    ...
    public function test_a_failure_on_a_later_page_does_not_advance_the_checkpoint(): void
    {
        ...
        Http::fake([
            self::TOKEN_URL => Http::response(['access_token' => 'tok', 'expires_in' => 3599]),
            self::INVOICES_URL.'*' => function (Request $request) use ($rows, &$calls) {
                $calls++;
        ...
        $this->artisan('bc:import-sales-invoices', ['--page-size' => 2])
    ...
    public function test_a_full_run_ignores_the_checkpoint(): void
    {
        ...
        $this->artisan('bc:import-sales-invoices', ['--page-size' => 200, '--full' => true])
    ...
    {
        $this->checkpoint()->update(['last_modified_at' => '2026-12-01 00:00:00.000']);
        $this->fakeInvoices($this->rows(2, '2026-06-15T00:23:21.653Z'));
    ...
    public function test_number_queues_only_that_invoice(): void
    {
        Http::fake([
            self::TOKEN_URL => Http::response(['access_token' => 'tok', 'expires_in' => 3599]),
            self::INVOICES_URL.'*' => Http::response(['value' => [$this->row(2)]]),
        ]);

        $this->artisan('bc:import-sales-invoices', ['--number' => 'INV000002'])
            ->expectsOutputToContain('checkpoint was left unchanged')
            ->assertExitCode(0);
    ...
    public function test_number_asks_business_central_for_that_number_only(): void
    {
        Http::fake([
            self::TOKEN_URL => Http::response(['access_token' => 'tok', 'expires_in' => 3599]),
            self::INVOICES_URL.'*' => Http::response(['value' => []]),
        ]);

        $this->artisan('bc:import-sales-invoices', ['--number' => "INV'1"])->assertExitCode(0);

        Http::assertSent(function (Request $request): bool {
            if (str_starts_with($request->url(), self::INVOICES_URL)) {
                $this->assertSame("number eq 'INV''1'", $request['$filter']);
            }
    ...
    {
        $this->artisan('bc:import-sales-invoices', ['--page-size' => 0])
    ...
    {
        $this->fakeInvoices($this->rows(1));

        $this->artisan('bc:import-sales-invoices', ['--force' => true])
    ...
    public function test_force_passes_the_flag_to_every_job(): void
    {
        ...
        $this->artisan('bc:import-sales-invoices', ['--full' => true, '--force' => true])
            ->expectsConfirmation('Continue?', 'yes')
            ->assertExitCode(0);

        Queue::assertPushed(
            ImportBcSalesInvoice::class,
            fn (ImportBcSalesInvoice $job): bool => $job->force === true,
        );
    }
    ...
    public function test_a_normal_run_does_not_force(): void
    {
        ...
        Queue::assertPushed(
            ImportBcSalesInvoice::class,
            fn (ImportBcSalesInvoice $job): bool => $job->force === false,
        );
    }
}
