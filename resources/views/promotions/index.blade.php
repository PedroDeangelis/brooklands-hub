...
<x-layout title="Promotions">
    <div class="flex flex-col gap-1.5">
        <h1 class="text-[32px] font-extrabold tracking-tight">Promotions</h1>
        <p class="text-[15px] text-ink-muted">
            <span class="tabular-nums">{{ number_format($campaigns->total()) }}</span>
            @if ($filter->isActive())
                matching {{ Str::plural('promotion', $campaigns->total()) }}
            @else
                campaigns imported from Business Central
            @endif
        </p>
    </div>

    <form method="GET" action="{{ route('promotions.index') }}" class="flex items-center gap-3">
        <div class="flex h-11.5 grow items-center gap-2.5 rounded-xl border border-line bg-surface px-3.5
            ...
            <label class="sr-only" for="list-search">Filter by code or promotion name</label>
            <input id="list-search" type="search" name="{{ CampaignFilter::PARAM_SEARCH }}" value="{{ $filter->search }}"
                   placeholder="Filter by code or promotion name"
        ...
        @foreach ($filter->without(CampaignFilter::PARAM_SEARCH) as $param => $value)
    ...
    <div class="flex flex-wrap items-center gap-2" aria-label="Filter by status">
        @foreach (CampaignStatus::cases() as $case)
            <a href="{{ route('promotions.index', $filter->without(CampaignFilter::PARAM_STATUS) + [CampaignFilter::PARAM_STATUS => $case->value]) }}"
               @class([
        ...
        <div class="flex flex-wrap items-center gap-2.5" aria-label="Active filters">
            ...
            @foreach ($filter->chips() as $chip)
                <span class="inline-flex items-center gap-2 rounded-full border border-line bg-surface py-1 pr-1.5 pl-3 text-[13px]">
                    ...
                    <a href="{{ route('promotions.index', $filter->without($chip['param'])) }}"
            ...
            <a href="{{ route('promotions.index') }}" class="text-[13px] font-semibold text-brand-ink hover:text-brand-deep">
                Clear filters
            </a>
        </div>
    @endif

    <section aria-label="Promotion table" class="flex flex-col rounded-[14px] border border-line bg-surface p-3">
            ...
            <table class="w-full min-w-5xl border-collapse text-sm">
                <thead>
                    <tr class="bg-fill-strong text-left text-[13px] font-medium text-ink-muted">
                        <th class="h-10.5 rounded-l-[9px] px-3.5 font-medium">Code</th>
                        <th class="px-3.5 font-medium">Promotion</th>
                        <th class="px-3.5 font-medium">Status</th>
                        <th class="px-3.5 font-medium">Starts</th>
                        <th class="px-3.5 font-medium">Ends</th>
                        <th class="px-3.5 text-right font-medium">Customers</th>
                ...
                <tbody>
                    @forelse ($campaigns as $campaign)
                        @php
                            ...
                            $delivery = DeliveryStatus::forCampaign($campaign->websiteSyncRecord);
                            $audience = count($campaign->customers ?? []);
                        @endphp

                        <tr class="border-b border-line-soft hover:bg-[#f7faf8]">
                            <td class="h-13.5 px-3.5 font-mono">
                                <a href="{{ route('promotions.show', $campaign) }}"
                                   class="font-medium text-brand-ink hover:text-brand-deep">{{ $campaign->code }}</a>
                            </td>
                            <td class="max-w-0 truncate px-3.5">{{ $campaign->title() }}</td>
                            <td class="px-3.5">
                                <x-campaign-status-badge :status="$status" />
                            </td>
                            <td class="px-3.5 text-[13px] text-ink-muted tabular-nums">
                                {{ $campaign->starting_date?->format('d M Y') ?? '—' }}
                            </td>
                            <td @class([
                                'px-3.5 text-[13px] tabular-nums',
                                'text-ink-muted' => $status !== App\Campaigns\CampaignStatus::Ended,
                                'font-medium text-excluded-ink' => $status === App\Campaigns\CampaignStatus::Ended,
                            ])>
                                {{ $campaign->ending_date?->format('d M Y') ?? '—' }}
                            </td>
                            <td @class([
                                'px-3.5 text-right tabular-nums',
                                'text-ink-faint' => $audience === 0,
                            ])>{{ number_format($audience) }}</td>
                            <td class="px-3.5">
                                <x-status-badge :status="$delivery" />
                            </td>
                            <td class="px-3.5">
                                @php($present = $onWebsite === null ? null : ($onWebsite[$campaign->bc_id] ?? false))
                                @if ($present === null)
                                ...
                                @elseif ($present === false)
                                    <span class="inline-flex items-center gap-1.5 rounded-full bg-failed-soft px-2.5 py-1 text-xs font-semibold text-failed-ink"
                                          title="The ledger says delivered, but the website does not hold this promotion.">
                            ...
                            <td class="px-3.5 text-[13px] text-ink-muted tabular-nums">
                                {{ $campaign->bc_modified_at?->format('d M Y H:i') ?? '—' }}
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="9" class="px-6 py-14 text-center">
                                <span class="flex flex-col items-center gap-1.5">
                                    <span class="text-[15px] font-semibold">No promotions match</span>
