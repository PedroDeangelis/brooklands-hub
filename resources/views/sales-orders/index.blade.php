...
<x-layout title="Sales orders">
    <div class="flex flex-col gap-1.5">
        <h1 class="text-[32px] font-extrabold tracking-tight">Sales orders</h1>
        <p class="text-[15px] text-ink-muted">
            <span class="tabular-nums">{{ number_format($salesOrders->total()) }}</span>
            @if ($filter->isActive())
                matching {{ Str::plural('order', $salesOrders->total()) }}
            @else
                open orders imported from Business Central
            @endif
        </p>
    </div>

    <form method="GET" action="{{ route('sales-orders.index') }}" class="flex items-center gap-3">
        <div class="flex h-11.5 grow items-center gap-2.5 rounded-xl border border-line bg-surface px-3.5
            ...
            <label class="sr-only" for="list-search">Filter by order number, customer or PO number</label>
            <input id="list-search" type="search" name="{{ SalesOrderFilter::PARAM_SEARCH }}" value="{{ $filter->search }}"
                   placeholder="Filter by order number, customer or PO number"
        ...
        @foreach ($filter->without(SalesOrderFilter::PARAM_SEARCH) as $param => $value)
    ...
    <div class="flex flex-wrap items-center gap-2" aria-label="Filter by status">
        @foreach (SalesOrderStatus::cases() as $case)
            <a href="{{ route('sales-orders.index', $filter->without(SalesOrderFilter::PARAM_STATUS) + [SalesOrderFilter::PARAM_STATUS => $case->value]) }}"
               @class([
        ...
        <div class="flex flex-wrap items-center gap-2.5" aria-label="Active filters">
            ...
            @foreach ($filter->chips() as $chip)
                <span class="inline-flex items-center gap-2 rounded-full border border-line bg-surface py-1 pr-1.5 pl-3 text-[13px]">
                    ...
                    <a href="{{ route('sales-orders.index', $filter->without($chip['param'])) }}"
            ...
            <a href="{{ route('sales-orders.index') }}" class="text-[13px] font-semibold text-brand-ink hover:text-brand-deep">
                Clear filters
            </a>
        </div>
    @endif

    <section aria-label="Sales order table" class="flex flex-col rounded-[14px] border border-line bg-surface p-3">
            ...
            <table class="w-full min-w-5xl border-collapse text-sm">
                <thead>
                    <tr class="bg-fill-strong text-left text-[13px] font-medium text-ink-muted">
                        ...
                        <th class="px-3.5 font-medium">BC status</th>
                        <th class="px-3.5 font-medium">Order date</th>
                ...
                <tbody>
                    @forelse ($salesOrders as $salesOrder)
                        @php
                            ...
                            $delivery = DeliveryStatus::forSalesOrder($salesOrder->websiteSyncRecord);
                        ...
                        <tr class="border-b border-line-soft hover:bg-[#f7faf8]">
                            <td class="h-13.5 px-3.5 font-mono">
                                <a href="{{ route('sales-orders.show', $salesOrder) }}"
                                   class="font-medium text-brand-ink hover:text-brand-deep">{{ $salesOrder->number }}</a>
                            </td>
                            <td class="max-w-0 truncate px-3.5">{{ $salesOrder->customer_name ?: '—' }}</td>
                            <td class="px-3.5">
                                <x-sales-order-status-badge :status="$status" />
                            </td>
                            <td class="px-3.5 text-[13px] text-ink-muted">{{ $salesOrder->bc_status ?: '—' }}</td>
                            <td class="px-3.5 text-[13px] text-ink-muted tabular-nums">{{ $salesOrder->order_date?->format('d M Y') ?? '—' }}</td>
                            <td @class([
                                'px-3.5 text-right tabular-nums',
                                'text-ink-faint' => $lines === 0,
                            ])>{{ number_format($lines) }}</td>
                            <td class="px-3.5 text-right tabular-nums">{{ number_format((float) $salesOrder->total_amount_excluding_tax, 2) }}</td>
                            <td class="px-3.5">
                                <x-status-badge :status="$delivery" />
                            </td>
                            <td class="px-3.5">
                                @php($present = $onWebsite === null ? null : ($onWebsite[$salesOrder->bc_id] ?? false))
                                @if ($present === null)
                                ...
                                @elseif ($present === false)
                                    <span class="inline-flex items-center gap-1.5 rounded-full bg-failed-soft px-2.5 py-1 text-xs font-semibold text-failed-ink"
                                          title="The ledger says delivered, but the website does not hold this order.">
                            ...
                            <td class="px-3.5 text-[13px] text-ink-muted tabular-nums">
                                {{ $salesOrder->bc_modified_at?->format('d M Y H:i') ?? '—' }}
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="10" class="px-6 py-14 text-center">
                                <span class="flex flex-col items-center gap-1.5">
                                    <span class="text-[15px] font-semibold">No sales orders match</span>
                                    <span class="text-sm text-ink-muted">
                                        @if ($filter->isActive())
                                            Try a different search or clear the filters.
                                        @else
                                            No sales orders have been imported yet.
                                        @endif
                                    </span>
                                </span>
                            </td>
